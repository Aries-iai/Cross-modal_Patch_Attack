...
def run(data,
        ...
        conf_thres=0.000000001,  # confidence threshold
...
def parse_opt():
    ...
    parser.add_argument('--conf-thres', type=float, default=0.000000001, help='confidence threshold')
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        if opt.conf_thres > 0.000000001:  # https://github.com/ultralytics/yolov5/issues/1466
            LOGGER.info(f'WARNING: confidence threshold {opt.conf_thres} >> 0.000000001 will produce invalid mAP values.')
        run(**vars(opt))
